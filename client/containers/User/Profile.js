...
class Profile extends Component {
  ...
  getUserInfo = id => {
    ...
    axios.get('/prd/api/user/find?id=' + id).then(res => {
      _this.setState({
  ...
  updateUserinfo = name => {
    ...
    axios.post('/prd/api/user/update', params).then(
      res => {
        let data = res.data;
        if (data.errcode === 0) {
  ...
  updatePassword = () => {
    ...
    axios.post('/prd/api/user/change_password', params).then(
      res => {
        let data = res.data;
        if (data.errcode === 0) {
            ...
            <Col span={24}>
              {userinfo.uid === this.props.curUid ? (
                ...
                <div className="avatarImg">
                  <img src={`/prd/api/user/avatar?uid=${userinfo.uid}`} />
                </div>
              )}
            </Col>
          </Row>
...
class AvatarUpload extends Component {
  ...
  uploadAvatar(basecode) {
    axios
      .post('/prd/api/user/upload_avatar', { basecode: basecode })
      .then(() => {
  ...
  render() {
    ...
    let imageUrl = url ? url : `/prd/api/user/avatar?uid=${this.props.uid}`;
    // let imageUrl = this.state.imageUrl ? this.state.imageUrl : `/prd/api/user/avatar?uid=${this.props.uid}`;
        ...
        >
          <div>
            <Upload
              ...
              action="/prd/api/user/upload_avatar"

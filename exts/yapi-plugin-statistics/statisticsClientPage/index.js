...
class statisticsPage extends Component {
  ...
  // 获取统计数据
  async getStatisData() {
    let result = await axios.get('/prd/api/plugin/statismock/count');
  ...
  async getSystemStatusData() {
    let result = await axios.get('/prd/api/plugin/statismock/get_system_status');
    if (result.data.errcode === 0) {
  ...
  async getGroupData() {
    let result = await axios.get('/prd/api/plugin/statismock/group_data_statis');
    if (result.data.errcode === 0) {
      let statusData = result.data.data;
      statusData.map(item => {

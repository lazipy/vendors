...
export default class MockCol extends Component {
  ...
  handleOk = async caseData => {
    ...
    await axios.post('/prd/api/plugin/advmock/case/save', caseData).then(async res => {
  ...
  deleteCase = async id => {
    const interface_id = this.props.match.params.actionId;
    await axios.post('/prd/api/plugin/advmock/case/del', { id }).then(async res => {
      if (res.data.errcode === 0) {
  ...
  openMockCase = async (id , enable=true)=> {
    ...
    await axios.post('/prd/api/plugin/advmock/case/hide', {
      id,
      enable: !enable
    }).then(async res => {
      if (res.data.errcode === 0) {
